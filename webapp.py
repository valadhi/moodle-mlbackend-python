...
def get_model_dir(hashkey):
    ...
    backendid = get_request_value('backendid')
    uniquemodelid = get_request_value('uniqueid')
    dirhash = get_request_value(hashkey)

    # The dir in the server is namespaced by backendid, uniquemodelid and the
    # dirhash which determines where should the results be stored.
    modeldir = os.path.join(basedir, backendid, uniquemodelid, dirhash)
